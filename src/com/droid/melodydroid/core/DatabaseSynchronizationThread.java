...
public class DatabaseSynchronizationThread implements Runnable {
	...
	private void synchBlock() {
		synchronized (syncObject) {
			...
			Map<String, Melody> existingMelodies = new HashMap<String, Melody>();
			...
			deviceAllMelodiesMap = synchMelodies(deletedMelodies, newMelodies,
					existingMelodies.keySet());

			if (deletedMelodies != null && deletedMelodies.size() > 0) {
				removeDeletedMelodies(deletedMelodies, existingMelodies);
			}
	...
	private void displayNotificationStartDBUpdate(NotificationManager nm) {
		Notification notification = new Notification(
				R.drawable.syncicon_notification, "Refresh "
	...
	private NotificationManager displayNotificationEndDBUpdate(
		...
		Notification notification = new Notification(
				R.drawable.syncicon_notification,
...
//		for(String songStr : existingMelodies) {
//			Log.v("DatabaseSynchronizationThread", "existingMelodies: " + songStr);
//		}

//		for(String songStr : deviceAllMelodiesSet) {
//			Log.v("DatabaseSynchronizationThread", "deviceAllMelodiesSet: " + songStr);
//		}
	...
	private void populateMelody(Map<String, File> deviceAllMelodiesMap,
		...
		if (musicMetaDataSet == null) // perhaps no metadata
			...
			// melody.setFileName(melodyFile.getName());
	...
	private void removeDeletedMelodies(Set<String> deletedMelodies,
			Map<String, Melody> existingMelodies) {
		DataHelper dataHelper = new DataHelper(context);
		...
		while (deletedMelodiesIterator.hasNext()) {
			String fileAbsolutePath = existingMelodies.get(
					(String) deletedMelodiesIterator.next()).getFileName();
