...
public class MelodiesSuggestionProvider extends ContentProvider {
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		if (selectionArgs != null) {
			...
			Log.v("MelodiesSuggestionsProvider: ", "userQuery" + userQuery);
			selection = " lower(album) like '%" + userQuery
					...
					+ "%' OR lower(fileName) like '%" + userQuery + "%'";
		} else {
			userQuery = null;
			selection = null;
		}

		projection = new String[] { "album", "singer", "genre", "title",
				"year", "fileName" };
		...
		if (userQuery != null && selection != null) {
			cursor = qb.query(db, projection, selection, null, null,
					null, sortOrder);
		}
	...
	private boolean findMatch(String dbString, String userQuery) {
		...
		Log.v("MelodiesSuggestionProvider: ", "dbString: " + dbString);
		
//		String[] dbStringTokens = dbString.split(" ");
//
//		for (int i = 0; i < dbStringTokens.length; i++) {
//			Log.v("MelodiesSuggestionProvider: ", dbStringTokens[i]);
//			if (dbStringTokens[i].startsWith(userQuery))
//				return true;
//		}

		if(dbString.contains(userQuery))
			return true;
