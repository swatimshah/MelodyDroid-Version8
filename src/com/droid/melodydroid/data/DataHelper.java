...
public class DataHelper {
	...
	public Map<String, Melody> getCurrentMelodies() {

		Map<String, Melody> fileMap = new HashMap<String, Melody>();
		Cursor cursor = db.rawQuery(
				"select album, singer, genre, title, year, albumArt, fileName from "
						+ TABLE_NAME, null);

		if (cursor.moveToFirst()) {
			do {
				Melody melody = new Melody();
				melody.setAlbum(cursor.getString(0));
				melody.setArtist(cursor.getString(1));
				melody.setGenre(cursor.getString(2));
				melody.setMelodyTitle(cursor.getString(3));
				melody.setYear(cursor.getString(4));
				melody.setAlbumArt(cursor.getBlob(5));
				melody.setFileName(cursor.getString(6));
				fileMap.put(
						cursor.getString(6).substring(
								cursor.getString(6).lastIndexOf("/") + 1),
						melody);
			} while (cursor.moveToNext());
		}
		cursor.close();
